...
class Sentence(object):
    ...
    def match_predicted_to_gold_mention(self, pred_mention, is_event, relaxed_match):
        ...
        """
        gold_mentions = (
            self.gold_event_mentions if is_event else self.gold_entity_mentions
        )
        found = False

        for gold_mention in gold_mentions:
            if (
                pred_mention.mention_str == gold_mention.mention_str
                and pred_mention.start_offset == gold_mention.start_offset
                ...
                relaxed_match
                and self.same_head(pred_mention, gold_mention)
                and not gold_mention.has_compatible_mention
            ):  # not sure about the has_compatible_mention
                pred_mention.has_compatible_mention = True
                gold_mention.has_compatible_mention = True
                pred_mention.gold_mention_id = gold_mention.mention_id
                pred_mention.gold_tokens = gold_mention.tokens
                ...
                relaxed_match
                and self.i_within_i(pred_mention, gold_mention)
                and not gold_mention.has_compatible_mention
            ):
                pred_mention.has_compatible_mention = True
                gold_mention.has_compatible_mention = True
    ...
    def i_within_i(self, mention_i, mention_j):
        ...
        """
        if (
            mention_i.start_offset >= mention_j.start_offset
            and mention_i.end_offset <= mention_j.end_offset
            and len(
                set(mention_i.tokens_numbers).intersection(
                    set(mention_j.tokens_numbers)
                )
            )
            > 0
        ):
            return True
        if (
            mention_j.start_offset >= mention_i.start_offset
            and mention_j.end_offset <= mention_i.end_offset
            and len(
                set(mention_i.tokens_numbers).intersection(
                    set(mention_j.tokens_numbers)
                )
    ...
    def same_head(self, mention_i, mention_j):
        ...
        """
        if (
            mention_i.mention_head == mention_j.mention_head
            and len(
                set(mention_i.tokens_numbers).intersection(
                    set(mention_j.tokens_numbers)
                )
    ...
    def find_nearest_entity_mention(self, event, is_left, is_gold):
        ...
        """
        sent_entities = (
            self.gold_entity_mentions if is_gold else self.pred_entity_mentions
        )
        ...
        for entity in sent_entities:
            diff = (
                event_start_idx - entity.end_offset
                if is_left
                else entity.start_offset - event_end_idx
            )
            if diff < 0:
                continue
...
class Mention(object):
    ...
    def __init__(
        ...
        self.mention_id = "_".join(
            [doc_id, str(sent_id), str(self.start_offset), str(self.end_offset)]
        )
    ...
    def get_head_index(self):
        ...
        for token in self.tokens:
            if (
                token.get_token() == self.mention_head
                or self.mention_head in token.get_token()
            ):
...
class EventMention(Mention):
    ...
    def __init__(
        ...
        """
        super(EventMention, self).__init__(
            doc_id,
            sent_id,
            tokens_numbers,
            tokens,
    ...
    def __str__(self):
        ...
        return "{}_a0: {}_a1: {}_loc: {}_tmp: {}_{}".format(
            super(EventMention, self).__str__(), a0, a1, aloc, atmp, self.mention_id
        )
...
class EntityMention(Mention):
    ...
    def __init__(
        ...
        """
        super(EntityMention, self).__init__(
            doc_id,
            sent_id,
            tokens_numbers,
            tokens,
    ...
    def __str__(self):
        ...
        return "{}_a0-pred: {}_a1-pred: {}_loc-pred: {}_tmp-pred:" " {}_{}".format(
            super(EntityMention, self).__str__(),
            a0_pred,
            a1_pred,
            aloc_pred,
            atmp_pred,
...
class Cluster(object):
    ...
    def __repr__(self):
        ...
        for mention in self.mentions.values():
            mentions_strings.append(
                "{}_{}_{}".format(
                    mention.mention_str, mention.gold_tag, mention.mention_id
                )
            )
