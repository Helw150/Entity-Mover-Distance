...
parser.add_argument("--ecb_path", type=str, help=" The path to the ECB+ corpus")
parser.add_argument("--output_dir", type=str, help=" The directory of the output files")
parser.add_argument(
    "--data_setup",
    type=int,
...
def calc_split_statistics(dataset_split, split_name, statistics_file_name):
    ...
    with open(statistics_file_name, "a") as f:
        ...
        f.write(
            "Number of human participants mentions - {}\n".format(human_mentions_count)
        )
        f.write(
            "Number of non-human participants mentions - {}\n".format(
        ...
        f.write(
            "Number of non-continuous mentions - {}\n".format(
                non_continuous_mentions_count
            )
        )
        f.write(
            "Number of mentions with coref id = UNK - {}\n".format(
...
def save_gold_mention_statistics(
    ...
    calc_split_statistics(
        train_extracted_mentions, "Train set", os.path.join(args.output_dir, filename)
    )

    calc_split_statistics(
        dev_extracted_mentions, "Dev set", os.path.join(args.output_dir, filename)
    )

    calc_split_statistics(
        test_extracted_mentions, "Test set", os.path.join(args.output_dir, filename)
    )

    calc_split_statistics(
        all_data_mentions, "Total", os.path.join(args.output_dir, filename)
    )

    logger.info("Save mention statistics...")


def read_ecb_plus_doc(
    selected_sent_list,
    doc_filename,
    doc_id,
    file_obj,
        ...
        for child in within_doc_coref.iter():
            if child.tag == "INTRA_DOC_COREF":
                mention_coref_class = mid_to_event_tag_dict[
                    ...
                    1
                ]  # find the mention class of intra doc coref mention
                if mention_coref_class == "UNKNOWN_INSTANCE_TAG":
                    cls = "UNK"
                else:
                    mention_class = mid_to_tag[
                        within_doc_coref.find("source").get("m_id")
                    ]
    ...
    for token in root.findall("token"):
        tokens[token.attrib["t_id"]] = Token(
            token.text, token.attrib["sentence"], token.attrib["number"]
        )
    ...
    for mid in mid_to_tid_dict:
        ...
        is_continuous = (
            True
            if token_numbers == range(token_numbers[0], token_numbers[-1] + 1)
            else False
        )
...
def parse_selected_sentences(xml_to_sent_dict, parse_all, load_singletons, data_setup):
    ...
    if data_setup == 1:  # Yang setup
        ...
        train_topics = [
            i for i in range(1, 36) if i not in dev_topics
...
def main():
    ...
    if (
        ...
        parse_selected_sentences(
            xml_to_sent_dict={}, parse_all=True, load_singletons=False, data_setup=1
        )
    elif (
        args.data_setup == 2
        ...
        parse_selected_sentences(
            xml_to_sent_dict=xml_to_sent_dict,
            parse_all=False,
